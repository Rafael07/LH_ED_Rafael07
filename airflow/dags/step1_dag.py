...
with DAG(
    ...
    step1_extract_csv = BashOperator(
        task_id='extract_csv',
        bash_command=f"source {data_path}/meltano_dataloader/.venv/bin/activate && python {data_path}/src/meltano_script.py extract"
    )

    step1_extract_postgres = BashOperator(
        task_id='extract_postgres',
        bash_command=f"source {data_path}/meltano_dataloader/.venv/bin/activate && python {data_path}/src/meltano_script.py load"
    )
